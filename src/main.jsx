...
function App() {
  ...
  useEffect(() => {
    if (auth.connected && step === 1) loadRepos();
  }, [auth.connected, step]);
    ...
    <div className="app">
      <header>
        ...
        {auth.connected ? (
          <div className="account">
            ...
            <span className="account-name">{auth.user?.name || auth.user?.login}</span>
            <button className="github secondary" onClick={logout}><LogOut size={16} /> Sign out</button>
          </div>
        ) : (
      ...
      <main>
        ...
        <section className="steps">
          {steps.map((x, i) => (
            <div className={i === step ? "step active" : i < step ? "step done" : "step"} key={x}>
              <span>{i < step ? <CheckCircle2 size={16} /> : i + 1}</span>{x}
            </div>
          ))}
        </section>
        ...
        <section className="card">
          {auth.loading && (
            <div className="loading-state"><Loader2 className="spin" size={25} /><p>Checking GitHub connection…</p></div>
          )}

          {!auth.loading && step === 0 && !auth.connected && (
            <>
              <div className="iconbox"><Github /></div>
              <h2>Connect your GitHub project</h2>
          ...
          {!auth.loading && step === 1 && auth.connected && (
            <>
              <div className="section-head">
                <div><h2>Select your web app</h2><p>Choose a repository from your GitHub account.</p></div>
                <button className="icon-button" onClick={loadRepos} disabled={reposLoading} title="Refresh repositories"><RefreshCw className={reposLoading ? "spin" : ""} size={18} /></button>
              </div>
              ...
              {reposLoading ? (
                <div className="loading-state"><Loader2 className="spin" size={25} /><p>Loading your repositories…</p></div>
              ) : filteredRepos.length ? (
                <div className="repo-list">
                  {filteredRepos.map(repo => (
                    <button key={repo.id} className={"repo-item " + (selectedRepo?.id === repo.id ? "selected" : "")} onClick={() => selectRepo(repo)}>
                      <span className="repo-main"><b>{repo.fullName}</b><small>{repo.description || "No description"}</small></span>
                      <span className="repo-meta">{repo.language || "Web"} {repo.private ? "· Private" : "· Public"}</span>
                    </button>
                  ))}
                </div>
              ) : (
              ...
              <button className="primary" disabled={!selectedRepo} onClick={continueRepo}>Continue <ArrowRight size={18} /></button>
            </>
          )}
          ...
          {step === 3 && (
            <>
              <h2>Configure your APK</h2>
              <p>These Android settings are applied to the wrapper; your original web app UI is not rebuilt.</p>
              <label>App name</label>
              <input value={appName} onChange={e => setAppName(e.target.value)} />
              <label>Package ID</label>
              <input value={packageId} onChange={e => setPackageId(e.target.value.toLowerCase().replace(/[^a-z0-9.]/g, ""))} />
          ...
          {step === 4 && (
            <>
              <div className="success"><CheckCircle2 /></div>
              <h2>Build configuration ready</h2>
              <p>The selected repository and Android settings are ready for the GitHub Actions build pipeline.</p>
              <div className="buildinfo"><b>{appName}</b><span>{packageId}</span></div>
              <button className="primary" disabled><Download size={18} /> APK build pipeline coming next</button>
            </>
          )}
        </section>
